...
public class Vesti extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.setTheme(R.style.Vesti);
        getWindow().setNavigationBarColor(getResources().getColor(R.color.vesti));
        getWindow().setStatusBarColor(getResources().getColor(R.color.vesti));

        setContentView(R.layout.vesti);

        final LinearLayout layout = findViewById(R.id.linear_vesti);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    Document doc = Jsoup.connect("https://imi.pmf.kg.ac.rs/vesti").get();
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        for (int i = 0; i < arrHtml.size(); i++) {
                            ...
                            final WebView web = new WebView(Vesti.this);
                            ...
                            html = "<html><head>"
                                    + "<style type=\"text/css\">body{color:" + "#424242" + ";} a{color:#FF8400; text-decoration:none; font-weight:bold;}"
                                    + "</style></head>"
                                    + "<body>" + html + "</body></html>";

                            String slika_novi_link = "", slika_link = "";
                            if (!html.contains("src=\"http://imi.pmf.kg.ac.rs/")) {
                                while (html.contains("src=\"p")) {
                                    slika_link = html.substring(html.indexOf("src=\"pub/") + 5);
                                    slika_novi_link = "http://imi.pmf.kg.ac.rs/" + slika_link;
                            ...
                            if (html.contains("src=\"pub"))
                                html = html.replaceAll("src=\"", "src=\"https://imi.pmf.kg.ac.rs/");


                            web.loadDataWithBaseURL("", html, mimeType,
                                    encoding, "");

                            web.setBackgroundColor(getResources().getColor(R.color.topla2));
                            ...
                            if (signal_za_tranziciju[0] == 0) {
                                ImageButton loading = findViewById(R.id.vesti_loading);
