...
public class OglasnaFaks extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);

        this.setTheme(R.style.OglasnaFaks);
        getWindow().setNavigationBarColor(getResources().getColor(R.color.oglas_faks));
        getWindow().setStatusBarColor(getResources().getColor(R.color.oglas_faks));

        setContentView(R.layout.oglasna_faks);

        final LinearLayout layout = findViewById(R.id.linear_faks);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    Document doc = Jsoup.connect("https://www.pmf.kg.ac.rs/index.php/oglasna-tabla").get();
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        for (int i = 0; i < arrText.size(); i++) {
                            ...
                            final TextView title = new TextView(OglasnaFaks.this);

                            title.setBackground(getResources().getDrawable(R.drawable.border_oglasna_faks));
                            ...
                            html = "<html><head>"
                                    + "<style type=\"text/css\">body{color:" + "#424242" + ";} a{color:#DEAAFF; text-decoration:none; font-weight:bold;}"
                                    + "</style></head>"
                                    + "<body>" + html + "</body></html>";

                            final WebView web = new WebView(OglasnaFaks.this);

                            final String mimeType = "text/html";
                            final String encoding = "UTF-8";
                            web.loadDataWithBaseURL("", html, mimeType,
                                    encoding, "");
                            if (html.contains("http://www.pmf.kg.ac.rs/") && !html.contains("logopmf")) {
                            ...
                            if (signal_za_tranziciju[0] == 0) {
                                ImageButton loading = findViewById(R.id.oglasna_faks_loading);
